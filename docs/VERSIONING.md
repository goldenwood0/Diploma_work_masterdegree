...
и сервер; `sprint-2` — профиль, онбординг и локализация; `sprint-3` — каталог,
уроки и прогресс; `sprint-4` — упражнения, тесты и история попыток.
`sprint-5` — словарь, заметки и интервальное повторение.
Перед следующим спринтом проверять `git status` и сохранять незавершённые
...
Для отдельной работы использовать ветки `codex/<название>` и pull request.
