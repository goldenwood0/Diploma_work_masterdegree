    ...
    <section className="max-w-4xl mx-auto space-y-8">
      ...
      {error ? (
        ...
        <>
          ...
          <div className="bg-secondary rounded-2xl p-6 sm:p-8 space-y-4">
            ...
            {next && (
              <p>
                {next.progress.nextBlock}/{next.blockCount} {t("блоков")}
              </p>
            )}
            <button
              onClick={() =>
                next ? onNavigate("lesson", next.slug) : onNavigate("catalog")
              }
              className="inline-flex items-center gap-3 rounded-xl bg-primary text-primary-foreground px-6 py-3 font-semibold"
            >
              {t(
                next
                  ? next.progress.nextBlock
                    ? "Продолжить"
                    : "Начать урок"
                  : "Курсы",
              )}
