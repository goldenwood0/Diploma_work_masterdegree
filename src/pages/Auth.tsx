  ...
  return (
    <main className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <section className="w-full max-w-md bg-white rounded-3xl border border-border shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div><h1 className="text-2xl font-bold text-dark-green">{title}</h1><p className="mt-2 text-muted-foreground">Китайский язык — шаг за шагом.</p></div>
        {error && <p role="alert" className="rounded-xl bg-coral/10 p-3 text-dark-green">{error}</p>}
        {message ? <p role="status" className="rounded-xl bg-sage p-4">{message}</p> : (
          <form onSubmit={submit} className="space-y-4">
            <fieldset disabled={busy} className="space-y-4 disabled:opacity-60">
              {mode === 'register' && <label className="block font-medium">Имя<input name="name" autoComplete="name" required maxLength={80} className="mt-2 w-full rounded-xl border p-3" /></label>}
              ...
              {needsPassword && <label className="block font-medium">{mode === 'resetPassword' ? 'Новый пароль' : 'Пароль'}<input name="password" type="password" autoComplete={mode === 'login' ? 'current-password' : 'new-password'} required minLength={12} maxLength={128} className="mt-2 w-full rounded-xl border p-3" /><span className="text-sm text-muted-foreground">От 12 до 128 символов</span></label>}
              {mode === 'verifyEmail' && <p>Нажмите кнопку, чтобы подтвердить адрес из письма.</p>}
              <button className="w-full bg-primary text-primary-foreground rounded-xl p-3 font-bold" type="submit">{busy ? 'Подождите…' : button}</button>
            </fieldset>
          </form>
        )}
        <nav aria-label="Доступ к аккаунту" className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-primary">
          {mode !== 'login' && <a href="#/login" className="hover:underline">Войти</a>}
          {mode !== 'register' && <a href="#/register" className="hover:underline">Создать аккаунт</a>}
          <a href="#/forgot-password" className="hover:underline">Забыли пароль?</a>
          <a href="#/resend-verification" className="hover:underline">Повторить письмо подтверждения</a>
        </nav>
      </section>
    </main>
  );
